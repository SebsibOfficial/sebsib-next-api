...
if (process.env.NODE_ENV == 'dev' || process.env.NODE_ENV == 'test') {
	mongoose.connect(process.env.TEST_DB_URL)
		...
		.then(() => app.listen(3004, () => console.log("API @ 3000 & DB @ " + process.env.TEST_DB_URL))) // Connect to the Database
}
else {
	mongoose.connect(process.env.PROD_DB_URL, {
		...
		.catch(error => console.error(error))
		.then(() => app.listen(3004, () => console.log("API @ 3000 & DB @ " + process.env.PROD_DB_URL))) // Connect to the Database
}
